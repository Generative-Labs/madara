...
use ethers::types::{Address, Filter, H256, I256, U256, Log};
use log::debug;
...
impl EthereumStateFetcher {
    pub fn new(
        ...
        Ok(Self { http_provider: provider, core_contract, verifier_contract,
			memory_page_contract, eth_url_list, current_provider_index:Arc::new(Mutex::new(current_provider_index))})
    }

	pub async fn get_logs_retry(&mut self, filter: &Filter) -> Result<Vec<Log>, Error> {
		let strategy = ExponentialBackoff::from_millis(100)
			.max_delay(Duration::from_secs(10))
			.factor(2); //NonZeroU64::new(10).unwrap()

		let mut retries = 0;
		loop {
			let current_index = self.current_provider_index.lock().map_err(|e| Error::Other(e.to_string()))?;

			let current_provider_url = &self.eth_url_list[*current_index];
			let provider = Provider::<Http>::try_from(current_provider_url)
				.map_err(|e| Error::L1Connection(e.to_string()))?;
    ...
    pub async fn query_memory_page_fact_continuous_logs(
        ...
        loop {
            ...
            let logs = self.get_logs_retry
				(&filter).await.map_err(|e| Error::L1Connection(e.to_string()))?;
    ...
    pub async fn query_state_diff<B, C>(&mut self, state_update: &StateUpdate, client: Arc<C>) -> Result<FetchState, Error>
    where
        B: BlockT,
        C: ProvideRuntimeApi<B> + HeaderBackend<B>,
        C::Api: StarknetRuntimeApi<B>,
