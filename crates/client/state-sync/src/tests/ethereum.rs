...
use ethers::types::{Address, H256,Filter};
...
async fn test_fetch_and_decode_state_diff() {
    ...
    let fetcher = EthereumStateFetcher::new( contract_address,
								   verifier_address,
								   memory_page_address,
								   eth_url_list).unwrap();
...
async fn test_sync_state_diff_from_l1() {
    ...
    let fetcher = EthereumStateFetcher::new(contract_address, verifier_address, memory_page_address,eth_url_list).unwrap();
...
async fn test_get_logs_retry() {
	...
	let mut client =
		EthereumStateFetcher::new(contract_address,
								  verifier_address,
								  memory_page_address,
								  eth_url_list).unwrap();
